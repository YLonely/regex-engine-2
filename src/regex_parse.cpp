...
/*
        ...
        RE::= simple-RE union
        union::= "|" simple-RE union | \epsilon
        simple-RE::= basic-RE concatenation
        concatenation::= basic-RE concatenation | \epsilon
...
using lw_regex::arguement_error;
using lw_regex::syntax_error;
using std::make_pair;
...
void set_item(shared_ptr<SetNode>& set) {
    ...
    while (!is_metachar(regex[index]) || regex[index] == '\\') {
        if (regex[index] == '\\') {
            ...
            switch (regex[index]) {
                ...
                case 'd':
                    ...
                    break;
                case 'D':
                    set->add_set_range((wchar_t)0, '0' - 1)
                        .add_set_range('9' + 1, (wchar_t)65535);
                    setchar = true;
                    break;
                case 's':
                    set->add_set_range(L'\t')
                    ...
                    break;
                case 'S':
                    set->add_set_range((wchar_t)0, (wchar_t)8)
                        .add_set_range((wchar_t)11)
                        .add_set_range((wchar_t)14, (wchar_t)65535);
                    setchar = true;
                    break;
                case 'w':
                    set->add_set_range(L'a', L'z')
                        .add_set_range(L'A', L'Z')
                        .add_set_range(L'0', L'9')
                        .add_set_range(L'_');
                    setchar = true;
                    break;
                case 'W':
                    set->add_set_range((wchar_t)0, '0' - 1)
                        .add_set_range('9' + 1, 'A' - 1)
                        .add_set_range('Z' + 1, (wchar_t)94)
                        .add_set_range((wchar_t)96)
                        .add_set_range((wchar_t)123, (wchar_t)65535);
                    setchar = true;
                    break;
                default:
                    c1 = regex[index];
        ...
        if (match('-')) {
            if (setchar)
                throw arguement_error(L"Wrong argument in \"[a-b]\"", regex,
                                      index);
            if (regex[index] == '\\') {
                c2 = regex[++index];
                if (c2 == 'd' || c2 == 'D' || c2 == 's' || c2 == 'S' ||
                    c2 == 'w' || c2 == 'W')
                    throw arguement_error(L"Wrong argument in \"[a-b]\"", regex,
                                          index);
                switch (c2) {
...
void functional_char(node_ptr& node) {
    ...
    switch (ch) {
        ...
        case 'd':
        case 'D':
            set = ch == 'd' ? make_shared<SetNode>(true)
                            : make_shared<SetNode>(false);
            set->add_set_range(L'0', L'9');
            break;
        case 's':
        case 'S':
            set = ch == 's' ? make_shared<SetNode>(true)
                            : make_shared<SetNode>(false);
            set->add_set_range(L'\t')
                .add_set_range(L'\n')
                .add_set_range(L'\r')
        ...
        case 'w':
        case 'W':
            set = ch == 'w' ? make_shared<SetNode>(true)
                            : make_shared<SetNode>(false);
            set->add_set_range(L'a', L'z')
                .add_set_range(L'A', L'Z')
                .add_set_range(L'0', L'9')
...
node_ptr elementary_re() {
    ...
    */
    auto is_functionalchar = [](const wchar_t& c) -> bool {
        switch (c) {
            case 't':
            case 'n':
            case 'r':
...
pair<int, int> range() {
    ...
    if (isdigit(regex[index]))
        ...
        throw arguement_error(L"Missing argument next to the '{'.", regex,
                              index);
    ...
    if (match('}'))
        ...
        if (match('}'))
            if (num1 >= num2 && num2 != -1)
                throw arguement_error(
                    L"In the operation \"{num1,num2}\",num1 must be smaller "
                    L"than num2.",
                    regex, index);
            else
        ...
        throw syntax_error(L"Wrong symbol in the operation \"{num1,num2}\".",
                           regex, index);
}
...
node_ptr basic_re() {
    ...
    if (match('{')) {
        if (!ele)
            throw arguement_error(L"Missing arguement before '{' '}'", regex,
                                  index);
        if (match('}'))
            throw arguement_error(L"Missing arguement in {min,max}", regex,
                                  index);
