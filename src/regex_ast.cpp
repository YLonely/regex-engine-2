...
/*
        This function will
        1.merge the duplicate characters in the SetNode.	i.e [aba-d] will
   be transfered into [a-d]. 2.reverse the negetive set to positive character
   range.
*/
void SetNode::merge() {
    if (set.size() != 1) {
        sort(set.begin(), set.end(),
             [](const pair<wchar_t, wchar_t>& p1,
                const pair<wchar_t, wchar_t>& p2) {
        ...
        for (auto it1 = set.begin(), it2 = set.begin() + 1;;) {
            if (it2 != set.end() && (it2 - 1)->second + 1 >= it2->first) {
                ...
                if (count > 1) {
                    temp1 = it1->first;
                    temp2 = it1->second > (it2 - 1)->second ? it1->second
                                                            : (it2 - 1)->second;
